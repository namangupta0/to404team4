...
weekdayData <- subset(masterdata, subset = (masterdata$numWeekday != "7" & masterdata$numWeekday != "1"))
weekendData <- subset(masterdata, subset = (masterdata$numWeekday == "7" | masterdata$numWeekday == "1"))
...
uniqueStarts <- as.data.frame(table(masterdata$start.station.name))
uniqueEnds <- as.data.frame(table(masterdata$end.station.name))
completeFrame <- data.frame(masterdata$start.station.name)
completeFrame <- unique(completeFrame)
completeFrame <- na.omit(completeFrame)
completeFrame$numStarts <- uniqueStarts$Freq[match(completeFrame$masterdata.start.station.name, uniqueStarts$Var1)]
completeFrame$numEnds <- uniqueEnds$Freq[match(completeFrame$masterdata.start.station.name, uniqueEnds$Var1)]


#Calculate Difference
uniqueStartsWeek <- as.data.frame(table(weekdayData$start.station.name))
uniqueEndsWeek <- as.data.frame(table(weekdayData$end.station.name))
uniqueStartsWeekend <- as.data.frame(table(weekendData$start.station.name))
uniqueEndsWeekend <- as.data.frame(table(weekendData$end.station.name))

completeFrame$numStartsWeek <- uniqueStartsWeek$Freq[match(completeFrame$masterdata.start.station.name, uniqueStartsWeek$Var1)]
completeFrame$numEndsWeek <- uniqueEndsWeek$Freq[match(completeFrame$masterdata.start.station.name, uniqueEndsWeek$Var1)]
...
head(weekList$masterdata.start.station.name, 10)



# Top 10 stations with deficit during the week
tail(weekList$masterdata.start.station.name, 10)
completeFrame$numStartsWeekend <- uniqueStartsWeekend$Freq[match(completeFrame$masterdata.start.station.name, uniqueStartsWeekend$Var1)]
completeFrame$numEndsWeekend <- uniqueEndsWeekend$Freq[match(completeFrame$masterdata.start.station.name, uniqueEndsWeekend$Var1)]
...
weekendList <- na.omit(weekendList)
weekendList <- arrange(completeFrame, desc(differenceWeekend))
Top10GainWeekend<- head(weekendList, 10) 

# Top 10 stations with deficit during the weekend
Top10DeficitWeekend <- tail(weekendList, 10)
...
### Top Deficit during Week 

```{r}
TenDeficitWeek <- ggplot(TopTenDeficitPM, aes(reorder(masterPM.start.station.name,  difference), difference)) +
    geom_col() +
    scale_x_discrete(label = function(x) stringr::str_trunc(x, 20)) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
TenDeficitAfternoon
...
pie(UsersBroadway, labels = c("Customers", "Subscribers"), main = "User Type Breakdown: Broadway & E 22 St")
