...
numMale <- nrow(masterdata[masterdata$gender == "Male", ])
# Number of Females in Sample
numFemale <- nrow(masterdata[masterdata$gender == "Female", ])
# Number of Unknowns in Sample
numUnknown <- nrow(masterdata[masterdata$gender == "Unknown", ])
# Total People in Sample
totalPeople <- numMale + numFemale + numUnknown
```

`r numMale / totalPeople * 100`% of Riders are Male
`r numFemale / totalPeople * 100`% of Riders are Female
`r numUnknown / totalPeople * 100`% of Riders are Unknown
...
Some stations have significantly higher trip duration than the expected average of `r mean(masterdata$tripduration)` seconds. This may signal that at some of these stations bikes are being lost or possibly not returned.
...
### Plots of Different Start and End Stations
```{r}
masterdata1 <- masterdata
masterdata1$dayofweek <- weekdays(masterdata1$startDate)

#first, let's create a new column for our function
masterdata1$diffstation <- ifelse(as.character(masterdata1$start.station.name) == as.character(masterdata1$end.station.name),TRUE,FALSE)

#now, let's plot some critical differences
df <- masterdata1 %>% group_by(usertype) %>% count(diffstation) %>% mutate(Percent = n / sum(n)*100)
ggplot(df, aes(x = usertype, y = Percent, fill = diffstation))+
    geom_bar(stat = "identity")+
    geom_text(aes(label = paste(Percent,"%"), y = Percent), 
          position = position_stack(vjust = 0.5))+
    coord_flip()+
    labs(x = "User Type", y = "Percentage",fill = "Different Station?")

#now, let's plot some critical differences
df <- masterdata1 %>% group_by(dayofweek) %>% count(diffstation) %>% mutate(Percent = n / sum(n)*100)
ggplot(df, aes(x = dayofweek, y = Percent, fill = diffstation))+
      geom_bar(stat = "identity")+
      geom_text(aes(label = paste(Percent,"%"), y = Percent), 
          position = position_stack(vjust = 0.5))+
  coord_flip()+
  labs(x = "Day Of Week", y = "Percentage",fill = "Different Station?")

#i want to map by gender as well
df <- masterdata1 %>% group_by(gender) %>% count(diffstation) %>% mutate(Percent = n / sum(n)*100)
ggplot(df, aes(x = gender, y = Percent, fill = diffstation))+
  geom_bar(stat = "identity")+
  geom_text(aes(label = paste(Percent,"%"), y = Percent), 
          position = position_stack(vjust = 0.5))+
  coord_flip()+
  labs(x = "Gender", y = "Percentage",fill = "Different Station?")
```
